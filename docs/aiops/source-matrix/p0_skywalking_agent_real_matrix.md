...
SkyWalking Agent 生态必须作为 FindX Agent 管理中心的一等能力接入：

- 不是链路监控页面的附属说明。
- 不是单纯包名清单。
- 不是 P5 后续优化才补的边角功能。

FindX 用户侧统一显示 FindX Agent / FindX Browser Agent / FindX 网关探针等命名；外部来源名称只允许留在本矩阵、合规来源登记、归档和开发证据中。

## 2. 已确认源码

| 本地路径 | 状态 | 作用 |
| --- | --- | --- |
| `D:\项目迁移文件\平台源码\skywalking-booster-ui-main` | `SOURCE_PRESENT` | 链路监控前端结构、路由、状态流、GraphQL 调用事实源 |
| `D:\项目迁移文件\平台源码\skywalking-master` | `SOURCE_PRESENT` | OAP/query protocol、WebApp proxy、后端查询语义事实源 |

独立 Agent 仓库如果未落地到 `D:\项目迁移文件\平台源码`，状态必须保持 `BLOCKED_LOCAL_SOURCE_MISSING`，不得用静态清单冒充实现。

## 3. Agent 能力包矩阵

| 能力包 | 上游源码事实源 | 当前本地状态 | FindX 用户侧命名 | 必须记录的制品/安装形态 | 数据到达验收 |
| --- | --- | --- | --- | --- | --- |
...
| 安装向导 | 选择主机/服务/Kubernetes workload、选择探针、生成安装计划、凭据引用、安装前检查、维护窗口、风险提示 | 直接拼接真实凭据；跳过安装前检查 |
| 远程安装 | Linux SSH/shell/systemd/Docker；Windows WinRM/PowerShell/Service/IIS；Kubernetes Helm/Operator/Webhook/DaemonSet/Sidecar/InitContainer | 只支持单机 shell；忽略 Windows/Linux/K8s 差异 |
| 配置模板 | OAP endpoint、service name、instance name、namespace、environment、采样、插件 include/exclude、日志关联、Trace propagation、TLS/proxy、标签、资源限制 | 配置不可编辑或只是静态说明 |
...
| 数据到达验证 | Trace、Metric、Log correlation、RUM、网关 Trace、Profiling 相关信号逐项验证 | 未验证数据到达就显示成功 |
| 版本治理 | 语言版本、运行时版本、Agent 版本、插件版本、配置版本、兼容风险、升级建议、已知问题、回滚目标 | 只给升级按钮，不展示风险 |
| 审计与证据链 | 包下载、签名校验、安装脚本、远程命令、配置变更、重启、回滚、卸载、数据到达证据进入 Evidence Chain | 日志输出真实 token、完整 DSN、Cookie 或会话 ID |

## 5. API / 数据契约门禁

进入代码实现前，必须完成以下契约设计并打标：
...
真实凭据必须只通过 `credential_ref`、`<TOKEN>`、`<DB_DSN>`、`<API_KEY>` 等占位符或引用表达。API、日志、文档和浏览器错误态不得输出真实 token、Cookie、完整连接串、SSH 私钥、Bearer 值或会话 ID。
...
| Trace 详情 | Agent 数据到达验证 | Trace 缺失或断点异常时定位探针加载、采样、OAP 连通和配置版本 |
| 拓扑 | 网关/服务探针状态 | Nginx Lua/Kong/应用探针的节点状态可解释拓扑缺口 |
| 链路告警 | 安装/配置任务证据 | 告警复盘可引用安装、升级、配置下发、回滚、数据到达证据 |
...
| 多语言 Agent 独立源码 | `BLOCKED_LOCAL_SOURCE_MISSING` | 本地尚未确认 `skywalking-java`、`skywalking-python`、`skywalking-nodejs`、`skywalking-php`、`skywalking-go`、`skywalking-rust`、`skywalking-ruby`、`skywalking-nginx-lua`、`skywalking-kong`、`skywalking-client-js` |
| FindX Agent 包实现 | `BLOCKED_BY_SOURCE_MATRIX` | 包仓库、安装计划、配置模板、心跳、数据到达、升级回滚契约未完成 |

P5 完成前必须用 MCP/Playwright 覆盖：
...
未补齐源码、契约、页面结构、MCP/Playwright 真实回归和敏感信息扫描前，不允许把 SkyWalking Agent 到 FindX Agent 控制面声明为完成。
