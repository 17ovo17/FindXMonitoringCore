...
- [运行态 DOM 证据目录](evidence/)

## 硬规则

- 没有源码路径、路由、核心组件、API、状态流、按钮动作、空态、错误态、权限态，不允许编码。
- 没有运行态 DOM / MCP 浏览器证据，不允许把页面声明为一比一完成。
- 独立源码未落地时只能标记 `BLOCKED_LOCAL_SOURCE_MISSING` 或 `BLOCKED`，不能用静态清单代替实现。
- 用户侧命名必须使用 FindX / FindX Agent；外部来源名称只允许留在本目录、合规登记、归档和开发证据中。
- SkyWalking Agent 是一等主线，必须在 P0 记录 Java、Python、Node.js、PHP、Go、Rust、Ruby、Nginx Lua、Kong、Browser Client JS 的来源、状态、包形态、安装方式、配置项和数据到达验收。
