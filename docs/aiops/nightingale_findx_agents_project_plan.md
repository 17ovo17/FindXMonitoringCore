...
| P1：告警核心与 Dashboard | evaluator、current/history event、规则试跑、版本回滚、Dashboard、模板中心、通知、权限、团队、值班、订阅、静默。 | **进行中**：P1 告警后端闭环部分已完成，P1-BE-1 `82ea50d`、P1-BE-2 `1b37905`、P1-BE-3 `1c4045e` 已推送；Dashboard、模板、通知、权限、团队、值班、订阅、静默仍待实现。 |
...
5. 下一步从 P1-FE-1 `/monitor` 工作台与 API 封装，或 P1-BE-4 notification/silence/subscription/oncall/pipeline/permission/audit 基座开始。
6. P1 剩余阶段完成 Dashboard、模板中心、安装 diff、回滚和漂移检测。
7. P1 剩余阶段完成通知、静默、订阅、值班、事件流水线、权限审计和团队/多租户闭环。
8. P2 基于 Categraf 建设 `findx-agents` 统一发行版。
9. P2 融合 Catpaw 授权衍生 inspector/diagnose/session/tool registry/MCP bridge/remote security。
10. P2 完成 FACP v1、三层配置模型、capability catalog、ToolDefinition、巡检模板、离线队列、安全审批和受控修复执行底座。
11. P3 实现 AI 问诊 evidence chain 和自动修复闭环。
12. 每个稳定切片完成权限、审计、文档、测试基准、WSL 构建和 Git 落库。
...
目标分：**>= 95**。本版计划自评分：**90/100**。

评分口径：本次只评价正式立项文档的完整性和可执行性，不把尚未实现的 P1/P2/P3 能力折算成运行成果。ops-diagnostician 审计给出的 82/100 主要扣分集中在 Nightingale 能力对标不够矩阵化、FACP v1 停留在消息名、Catpaw 衍生合规缺少模板、评分偏乐观；本版已补齐这些文档闭环，但仍缺少对应后端、前端、Agent 和 QA 运行证据，因此不能恢复到 95+。

| 维度 | 分值 | 评分理由 |
| --- | --- | --- |
| 战略完整性 | 94 | 已明确 FindX Monitoring Core 为新监控核心平台，P0 已通过，P1 告警后端闭环部分已完成，P1 剩余域与 P2/P3 保持全功能路线，并把 Nightingale、Categraf、Catpaw 的角色纳入统一主线；仍需后续实现证明路线可持续。 |
| 可实施性 | 92 | 已新增 Nightingale 能力对标 Gap Matrix，覆盖 target、datasource、query、alert rule、event、dashboard、template、notification、silence、subscription、oncall、event pipeline、task、permission、audit、team、alert escalation、import/export、multi-tenant，并明确缺口、验收用例和优先级。 |
| 安全治理 | 91 | FACP v1 已补成协议契约，覆盖请求/响应、错误码、签名、nonce、timestamp、脱敏、审计、兼容版本、revoke 和 remediation 审批；但尚无 Agent 端与服务端实现验证。 |
| 生态复用 | 90 | Categraf inputs/provider/remote_write/heartbeat 作为采集底座；Catpaw 授权能力衍生为 inspector/diagnose/session/tool registry/MCP bridge/remote security；本版补齐 Catpaw 衍生合规模板，但 LICENSE/NOTICE 文件和商业分发复核还未实际落仓。 |
| 验证闭环 | 82 | P0 已由 commit `81b4531` 证明 QA 98/100 通过，Windows + WSL Go 测试/构建通过；P1-BE-1 `82ea50d`、P1-BE-2 `1b37905`、P1-BE-3 `1c4045e` 已记录为当前切片；P1-BE-3 QA 92/100 PASS，Windows/WSL `go test -count=1 ./...` 与 build 通过。扣分原因是本次为文档变更，未新增运行态代码、WSL 构建、API/UI 回归或 qa-tester 复测。 |
| 文档可执行性 | 93 | 文档给出 API、数据表、Nightingale Gap Matrix、FACP v1 协议契约、配置模型、Capability Schema、ToolDefinition、巡检模板、离线保护、安全审批、Git 策略和阶段边界；仍需随着 API_CONTRACT_CHANGE、DATA_CHANGE 和测试基准持续回填。 |

扣分项：

- P1 后端告警闭环虽已有运行证据，但 Dashboard、模板、通知、权限、团队、值班、订阅、静默、事件流水线、任务、导入导出、多租户仍未实现；P2/P3 仍只能给出全功能闭环设计和验收要求，不能提供运行证据。
