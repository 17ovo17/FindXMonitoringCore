...
- Categraf 插件配置必须在 Agent 管理中心体现为可选择、可下发、可远程修改的 FindX Agent 采集插件配置；真实远程修改契约缺失时显示 `BLOCKED_BY_CONTRACT`。
- Agent 管理中心展示的所有插件和监控能力都必须进入闭环验收；缺少 Windows、Linux、Kubernetes、SSH、WinRM、示例应用、OAP、Categraf、Catpaw 或日志/指标/Trace 环境时，先安装环境再验证，不允许用环境缺失跳过 UI 和数据到达验收。
...
- P5：完整生命周期，包仓库、离线包、签名、本机/远程/K8s 安装、配置下发、Categraf 插件配置远程修改、心跳、数据到达、升级、回滚、卸载。

前端验收不能只证明命令预览存在。Linux `curl -kfsSL`、Windows CMD `certutil -urlcache -f`、PowerShell `Invoke-WebRequest` 入口必须能连接安装任务、执行日志、服务状态、心跳和数据到达证据；远程安装、远程下发、卸载、配置下发、插件下发、升级、回滚和失败恢复必须有真实状态流或明确 `BLOCKED_BY_CONTRACT`。

SkyWalking Agent、Categraf、Catpaw 以及 FindX Agent 下所有监控插件必须覆盖 Windows/Linux 双实现、双验证；Kubernetes 场景按 Helm、Operator、DaemonSet、Sidecar、InitContainer、RBAC、TLS 和回滚单独闭环。`409 BLOCKED_BY_CONTRACT` 只能作为诚实阻断展示，不能作为安装、下发、卸载或数据到达成功。
...
未补齐源码、包契约、安装脚本、配置模板、真实安装/卸载/远程执行、心跳和数据到达验证前，只能显示 `BLOCKED` 或 `BLOCKED_BY_CONTRACT`。
