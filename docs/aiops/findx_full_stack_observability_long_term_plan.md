...
- 不允许只在对话中保留硬约束。任何代码、配置、依赖、路由、测试、验证、反卡死或需求边界变更，都必须同步维护任务板、执行日志和对应长期文档，防止上下文压缩或新会话后丢失约束。

## 3. 总体架构路线

FindX 前端采用 React-first 方向重构壳层和主要观测工作台。基础监控必须落到 Nightingale 同系 React 技术栈；若现有 Vue 页面仍承担历史功能，只能作为兼容桥、过渡承载或待迁移对象，按切片逐步下线或替换，不继续在弱化 Vue 页面上堆补丁。成熟源码若为 React 体系，优先使用同技术体系迁移组件结构、状态流和交互语义，以降低翻译损耗。

React-first 前端技术栈、Vue 退场边界、多源前端兼容策略、React Shell 职责、页面迁移方法、测试准出和多 agent 执行制度以 [FindX React-first 前端技术栈长期闭环计划](findx_react_first_frontend_long_term_plan.md) 为强制子计划。任何前端依赖、入口、路由、壳层、桥接或页面迁移变更，都必须同步该子计划。

当前主线优先级是 React-first 前端提级和前端功能闭环。P0-P4 的基础监控、组织权限/系统配置、CMDB 前端、链路监控前端、日志中心前端必须先形成可验收闭环，再进入 P5 FindX Agent 侧安装器、包仓库、远程安装、配置下发、心跳、数据到达、升级回滚和卸载等深度调优。Agent 管理中心在前端闭环期间只做必要的导航、页面入口、状态展示、契约设计和 `BLOCKED_BY_CONTRACT` 边界，不抢占前端主线。
...
编码前门禁参考：[FindX React-first 前端技术栈长期闭环计划](findx_react_first_frontend_long_term_plan.md)。该阶段必须明确 React 依赖、Vite 插件、Vue 兼容桥、React Shell、路由 alias、主题变量、权限上下文、错误态、品牌脱敏、WSL build 和 MCP 浏览器回归，不得只以 Windows build 或静态页面作为完成证据。
...
阶段门禁：P5 作为 Agent 侧深度调优阶段，必须等待 React-first 前端功能闭环达到可验收状态后再作为主线推进。前端未闭环前，不得把远程安装、离线包仓库、心跳、探针数据到达、升级回滚和卸载作为主任务抢占 P0-P4。
...
- 每次实现、配置、依赖、路由、验证、测试、反卡死处理或 P0/P1 风险处理，都必须同步维护 `.claude/codex-task-board.md` 和 `.claude/operations-log.md`。
- 每次长期方向、硬约束或架构边界变化，必须同步 `AGENTS.md`、`README.md`、`docs/aiops/README.md` 和本主计划。
- 每次前端技术栈、React/Vue 入口、桥接、路由、页面迁移或前端测试门禁变化，还必须同步 [FindX React-first 前端技术栈长期闭环计划](findx_react_first_frontend_long_term_plan.md)。
- 每个源码迁移切片必须维护对应源码矩阵或 `.claude/context-question-*.json`，记录源码路径、路由、组件、API、状态流、按钮动作、空态、错误态、权限态和 FindX 替换点。
- 文档未覆盖本轮改动时不得标记 PASS；只能标记 `BLOCKED`、`RISK` 或继续补齐文档。
- 任何切片结束前必须完成文档准出核对：任务板状态、执行日志、源码证据、影响面、验证结果、未覆盖项、下一步归属均已记录。
- 上下文压缩、新会话或恢复执行后，必须先读取 `AGENTS.md`、`README.md`、`docs/aiops/README.md`、本主计划、`.claude/codex-task-board.md` 和 `.claude/operations-log.md`，再继续派发或编码。
- 文档维护不是附属项，而是实现完成定义的一部分；没有文档闭环的代码、依赖、路由、配置、验证或风险处理一律不能进入 Git 门禁。
