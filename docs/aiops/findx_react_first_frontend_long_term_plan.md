...
当前执行优先级固定为：先完成 React-first 前端提级和前端功能闭环，再进入 Agent 侧深度调优。前端闭环至少包括基础监控、链路监控、日志中心、CMDB/Agent 在线前端视图、组织权限、系统配置、AI SRE 前端入口和对应 MCP 浏览器回归。Agent 侧安装器、离线包仓库、远程安装、配置下发、心跳、探针数据到达、升级回滚和卸载调优进入 P5 之后推进，不抢占 P0-P4 React 前端闭环。
...
完成 FindX Agent 包仓库、安装向导、本机脚本、远程安装、配置下发、心跳、数据到达、升级、回滚、卸载、审计和 Evidence Chain。本阶段不得早于 P0-P4 React 前端功能闭环启动为主线；在前端闭环前，只允许维护 Agent 管理中心的前端占位、源码矩阵、契约设计和明确 `BLOCKED_BY_CONTRACT` 状态，不进入探针生命周期深度调优。
...
- 下一步优先级为 React-first 前端功能闭环：先处理数据源契约风险并推进集成中心、指标查询、仪表盘、模板中心、告警、通知、链路监控、日志中心等前端同源迁移；若不能证明 per-item upsert 和 full round-trip 安全，则新增/编辑/删除必须标 `BLOCKED_BY_CONTRACT`，或先补后端契约。
- Agent 侧深度调优暂不抢占当前主线；只在前端闭环需要的导航、页面入口、状态展示和契约边界范围内处理。
