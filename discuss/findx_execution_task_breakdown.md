...
- 当前已有后端权限矩阵和前端导航改动均处于待验收状态；P0-Gate 未通过前，不得与任何 P0/P1/P3 功能切片直接混合提交。
- 子代理不得写入真实密钥、认证票据、Cookie、完整连接串、会话 ID；示例统一使用 `<TOKEN>`、`<API_KEY>`、`<DB_DSN>`、`<LOGIN_USER>`。

## 2. 当前工作区脏改动审计

当前工作区可分成 6 类脏改动。以下审计发现是后续执行的阻断输入，P0-Gate 未关闭前，不允许把这些文件混入业务切片。
...
P0-Gate 的特殊结论：当前前端导航不允许通过，直到命名风险和健康检查归属风险修正并有浏览器证据。健康检查必须归 AI SRE；平台治理只保留平台运行自检。
...
| 字段 | 内容 |
|---|---|
| 任务目标 | 冻结现有后端权限矩阵、前端导航、命名边界和健康检查归属；当前前端导航不允许通过，直到命名风险和健康检查归属风险修正。 |
...
| 敏感信息扫描 | 检查代码、日志、页面和接口响应是否出现真实密钥、认证票据、Cookie、完整连接串、会话 ID；允许 `<TOKEN>`、`<API_KEY>`、`<DB_DSN>`、`<LOGIN_USER>` 等占位符，允许既有文件名如 `ChangePassword.vue`；禁止真实密钥、裸 Bearer 值、真实浏览器会话凭据、真实完整 DSN、SSH 私钥和 `@tcp(` 连接串。 |
...
1. 先冻结 P0-Gate，并修正前端导航命名风险和健康检查归属风险。
