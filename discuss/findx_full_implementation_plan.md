...
> 文档性质：全阶段实施规划
> 当前状态：P0-Gate 已闭环；后续进入 P0-T1 前必须先通过剩余脏文件隔离门禁
> 本轮范围：实施计划文档的闭环更新，不实施业务代码；相关文档可按闭环需要提交。
> 日期：2026-05-05（UTC+8）
> 参考来源：成熟开源监控平台设计、主机巡检与 AI SRE 实践、现有 FindX 项目架构

## 0. P0-Gate 闭环状态

**闭环日期**：2026-05-05（UTC+8）

P0-Gate 已完成并闭环，当前可作为 P0-T1 的前置门禁基线。闭环证据如下：

- 规划基线提交：`f36927f docs: define FindX execution planning`。
- 权限与导航门禁提交：`b3a8893 feat: add FindX monitor permission gate and navigation`。
- Git 状态：已 push `origin/main`。
- QA 结论：92/100，PASS，无 P0/P1 阻断。

P0-Gate 闭环只代表权限门禁、导航命名和健康检查归属的前置收口已经通过审计，不代表 P0-T1 业务空间、P0-T2 资源组与主机资产、P0-T3 监控仪表盘等业务能力已经实现。

下一步进入 **P0-T1：业务空间**。开始前必须先执行“剩余脏文件隔离门禁”，确保后续业务空间切片不混入认证入口、远程配置控制面或旧监控工作台大改。

### 0.1 剩余脏文件隔离门禁

P0-T1 开始前，主代理必须先对以下剩余脏文件执行隔离门禁。处理方式只能是只读审计、隔离或后续独立切片处理；不得直接 revert 用户改动，也不得把这些文件混入 P0-T1 的写集、验证证据或 Git 门禁。

| 文件 | 隔离原因 | P0-T1 准入要求 |
|---|---|---|
| `web/src/views/ChangePassword.vue` | 认证入口，涉及改密流程和登录态边界。 | 不能混入 P0-T1；仅允许只读审计、隔离或后续认证入口独立切片处理。 |
| `web/src/views/Login.vue` | 认证入口，涉及登录流程、认证头和会话状态。 | 不能混入 P0-T1；仅允许只读审计、隔离或后续认证入口独立切片处理。 |
| `web/src/views/PluginConfig.vue` | 远程配置、命令和凭证相关页面，存在权限、审计、脱敏和执行边界安全风险。 | 不能混入 P0-T1；仅允许只读审计、隔离或后续 findx-agents 控制面/远程配置独立切片处理。 |
| `web/src/views/monitoring/MonitorWorkbench.vue` | 旧监控工作台大改，可能影响导航、监控入口和历史页面行为。 | 不能混入 P0-T1；仅允许只读审计、隔离或后续监控工作台独立切片处理。 |

P0-T1 准入条件：

- P0-Gate 闭环证据已确认，且无新增 P0/P1 阻断。
- 上述四个剩余脏文件已完成隔离记录，不进入 P0-T1 写集。
- P0-T1 只处理业务空间相关后端模型、存储、接口、权限边界和前端入口。
- 健康检查继续归 AI SRE；平台治理只保留平台运行自检。
- findx-agents 继续定位为采集器与主机接入控制面，不在 P0-T1 中扩展远程配置、命令执行或凭证处理。

## 1. 总览

本计划围绕 **FindX Core**、**findx-agents**、**AI SRE** 三条主线展开。FindX Core 负责可观测基础能力、资源治理、告警通知、团队权限和平台运行自检；findx-agents 负责采集器能力、主机接入、采集插件、配置下发、远程安装、调试、自升级、命令执行、自愈和单机巡检入口，并作为主机接入控制面承接 Agent 侧治理；AI SRE 负责主机健康检查、单机巡检、AI 问诊、RCA、证据链、自动修复和复盘闭环。
...
| P4 | 多信号数据与可观测底座 | 日志、Trace/APM、RUM、指标目录、智能告警、分析视图、多数据源 |
...
侧边栏只放运维主对象和高频工作台，不放动作型、记录型、配置型子能力；子能力必须收敛到所属页面的页签、抽屉、详情动作、模板导入或高级配置中，避免把同一业务对象拆成多个低频入口。不要把动作、模板子项、配置子项、记录子项、同一对象的不同视角拆成同级菜单。

“完整复刻成熟监控平台所有能力”指能力完整、契约完整、验证完整，不等于侧边栏一项一功能。侧边栏只放主对象和主工作台，子能力放到所属主对象页面内承载。
...
- 记录规则不是独立侧边栏入口，归入告警规则的高级配置、规则模板导入或预计算规则能力。
- 告警订阅不是独立侧边栏入口，归入事件中心、告警规则或通知策略中的关注与接收偏好；通知域只保留通知规则、通知媒介、消息模板、通知记录这些主对象。
...
POST   /api/v1/analysis-views
PUT    /api/v1/analysis-views/:id
DELETE /api/v1/analysis-views/:id
```

**能力范围**：

- 支持日志查询收藏与分析视图复用。
- 支持 Trace 查询收藏与跨信号分析视图复用。
- 支持资产过滤条件沉淀为分析视图。
- 支持告警事件过滤条件沉淀为分析视图。
- 支持团队内分享与权限隔离。
