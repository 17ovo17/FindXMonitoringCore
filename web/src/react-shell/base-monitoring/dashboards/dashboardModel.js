...
export const blockedContracts = {
  exportApi: `${BLOCKED_BY_CONTRACT}: 批量导出服务端契约未暴露，当前只导出已加载的脱敏配置。`,
  panelRender: `${BLOCKED_BY_CONTRACT}: Panel 查询、变量替换、渲染器、loading、empty 和 error 状态契约未完整暴露。`,
  panelEditor: `${BLOCKED_BY_CONTRACT}: 图表编辑器需要查询编辑器、渲染配置、变换、覆盖和审计契约。`,
  variableOptions: `${BLOCKED_BY_CONTRACT}: 动态变量选项、搜索、URL 联动和 Panel 查询替换契约未完整暴露。`,
  runtimeControls: `${BLOCKED_BY_CONTRACT}: 时间范围、刷新间隔和时区需要 Panel 查询、变量替换、URL 联动和自动刷新契约。`,
  importPanel: `${BLOCKED_BY_CONTRACT}: 从外部 Panel 导入需要解析、冲突检测和回滚契约。`,
  inspect: `${BLOCKED_BY_CONTRACT}: 查询参数检查需要 Panel 查询执行和响应检查契约。`,
  shareView: `${BLOCKED_BY_CONTRACT}: 分享视图需要分享 token 校验、过期控制、权限隔离和只读渲染契约。`,
}
