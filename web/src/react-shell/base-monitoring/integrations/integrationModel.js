...
export const blockedContracts = {
  components: '组件实体列表契约未暴露，当前仅展示仪表盘模板降级视图，不能判定组件实体完成。',
  systems: '系统集成新增、编辑、删除、排序和菜单显示状态已接入 FindX 后端写契约；嵌入打开和动态菜单嵌入仍待接入。',
  ...
  collect: '采集模板列表、分类、下发和回滚契约未暴露。',
  metric: '内置指标列表、指标详情和查询联动契约未暴露。',
  alert: '告警规则 payload 写入已接入 FindX 后端契约；导入、数据源替换和批量导出业务落地仍未暴露。',
  record: '记录规则模板列表、导入、启停和预计算契约未暴露。',
  payloadCreate: 'dashboard、collect、alert payload 新增已接入 FindX 后端写契约；metric、firemap、record 仍待接入。',
  ...
  payloadImport: 'payload 导入到业务组契约未暴露，当前不打开导入表单，也不调用非同源导入接口。',
  payloadContent: '当前记录缺少可用 payload.content，不能预览、导出或导入。',
  ...
  systemOpen: '系统集成嵌入打开契约未开放，当前不打开 iframe、WebView 或外部窗口。',
}
...
export const parsePayloadDraft = (draft, editing = false) => {
  ...
  if (!writablePayloadTypes.has(type)) return { error: `payload 类型 ${type || '-'} 暂未开放写入，待接入。` }
  if (!editing && id && !/^[A-Za-z0-9_.:-]{1,64}$/.test(id)) return { error: 'ID 只能包含字母、数字、点、下划线、冒号或短横线，最长 64 位。' }
...
export const systemDetailJson = (row) => safeJson({
  ...
  blocked_actions: {
    ...
    menu_embedding: '菜单嵌入仍由后端契约阻断，当前仅允许切换是否显示在菜单。',
  },
}, 20000)
...
export const buildFallbackComponents = (payloads) => {
  ...
  payloads.forEach((payload) => {
    ...
    const current = map.get(key) || {
      ...
      readme: '当前仅展示仪表盘模板的降级视图。组件实体契约未暴露，新增、编辑、删除、导入与系统集成仍待接入。',
      disabled: false,
      dashboardCount: 0,
      alertCount: 0,
      metricCount: 0,
