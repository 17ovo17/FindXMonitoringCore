...
export const LOG_BLOCKERS = {
  query: '通用 OTel 日志查询、分页、详情、导出和查询审计缺少 Logs Adapter 契约；当前仅 FindX 审计日志可查询。',
  fields: '字段发现、已选字段保存、字段值联想和字段类型统计缺少 Logs Adapter 契约。',
  context: '日志上下文、前后文游标、同实例和同 Trace 窗口缺少 Logs Adapter 契约。',
  aggregate: '通用生产日志聚合、分桶、分组、趋势图和取消请求缺少 Logs Adapter 契约；当前仅 FindX 审计日志可聚合。',
  live: '实时日志流、断线重连、背压、暂停和权限续期缺少 Logs Adapter 契约。',
  pipeline: '生产管道部署、生效、远程下发、回滚和审计契约尚未接入。',
  savedViews: '保存视图分享和跨来源加载仍缺少 Logs Adapter 契约。',
  traceLink: '日志 Trace 字段提取、链路详情深链和权限校验缺少 Logs Adapter 与 APM 联动契约。',
  agentLinkage: '日志采集能力包、主机 FindX Agent 心跳、数据到达和配置版本缺少 Logs-Agent 映射契约。',
}
...
export const formatLogError = error => {
  ...
  if ([404, 405, 409, 501, 503].includes(error?.status)) return redactText(error.message || '接口未开放。')
