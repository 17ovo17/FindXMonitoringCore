...
const BLOCKED_NOTE = '配置/插件下发仍为 BLOCKED_BY_CONTRACT；这里只展示被阻断的持久化审计记录和 safe _ref contract metadata，不表示真实执行。'
...
function Detail({ record }) {
  ...
  return (
    <div className='fx-agent-summary-row'>
      ...
      <Tags items={scopes.length ? scopes : ['scope: BLOCKED_BY_CONTRACT']} />
    ...
    <div className='fx-agent-table'>
      <table>
        ...
        <tbody>{REF_KEYS.map(key => {
          ...
          return <tr key={key}><td>{key}</td><td><Status ok={false}>{value ? '已提供 safe ref' : '缺失'}</Status></td><td><span className='fx-agent-muted'>{value ? redactRef(value) : 'BLOCKED_BY_CONTRACT'}</span></td></tr>
        })}</tbody>
      </table>
    </div>
  )
...
function PayloadPreview({ record }) {
  const rows = PAYLOAD_KEYS.map(key => ({ key, value: safeValue(record, key) || 'BLOCKED_BY_CONTRACT' }))
  return (
    <div className='fx-agent-table'>
      <table>
  ...
  const openDetail = id => {
    ...
    agentApi.getConfigRollout(id)
      .then(value => {
        ...
        else { setDetail(null); setError('BLOCKED_BY_CONTRACT: record detail 不是 blocked 配置/插件记录，已拒绝展示。') }
      })
      .catch(err => setError(formatAgentError(err)))
