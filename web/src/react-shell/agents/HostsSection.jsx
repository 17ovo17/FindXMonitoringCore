...
function PluginConfigTags({ pluginConfig }) { return <><div className='fx-agent-subtitle'>{T.pluginMutation}</div><Tags items={[`${T.format} ${pluginConfig.configFormat}`, `${T.provider} ${(pluginConfig.providerModes || []).join('/')}`, pluginConfig.remoteMutationStatus === '' ? T.mutationBlocked : T.pluginMutation]} /></> }
