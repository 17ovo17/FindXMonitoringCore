...
export function TemplatesSection() {
  ...
  const rollout = rolloutStrategy => {
    setBlocked(`BLOCKED_BY_CONTRACT: 统一配置模板页缺少真实 CMDB/主机 Agent 目标选择器，请从主机 Agent 的配置/插件下发抽屉选择真实目标后执行 ${rolloutStrategy}。`)
  }
  ...
  const buildRolloutPayload = rolloutStrategy => ({
    ...
    target_selector: 'BLOCKED_BY_CONTRACT: 在主机 Agent 抽屉中选择真实目标',
  })
      ...
      <div className='fx-agent-panel'>
        ...
        {selected.pluginConfig && <>
          ...
          <Tags items={[
            ...
            selected.pluginConfig.remoteMutationStatus === 'BLOCKED_BY_CONTRACT' ? '远程修改契约待开放' : '远程修改',
          ]} />
        </>}
