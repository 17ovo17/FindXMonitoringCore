      ...
      try {
        ...
        if (!cancelled) {
          if (resp && resp.metrics) {
            ...
            setBlocked('BLOCKED_BY_CONTRACT: 需要后端实现 /apm/overview?serviceId&metrics API 返回时序指标数据')
          }
        }
      } catch (err) {
        if (cancelled) return
      ...
      {METRICS.map(m => {
        ...
        if (!values.length) return (
          <div key={m.key} className='fx-metric-chart'>
            ...
            <Blocked>BLOCKED_BY_CONTRACT: 需要后端实现 /apm/overview 返回 {m.key} 时序数据</Blocked>
          </div>
        )
